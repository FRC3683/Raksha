...
import frc.robot.utils.swerve.swerve_kin2;
...
public class swerve_mech2d {
    ...
    public void update(Rotation2d pose_rotation, module_output[] desired_states, module_speed[] achieved_states) {
        rotation.setAngle(pose_rotation.getDegrees()+90.0);
        for(int i = 0; i < num_modules; ++i) {
            final Translation2d trans = module_translations[i];
            final module_output desired = desired_states[i];
            final module_speed actual = achieved_states[i];
            desired_module_states[i].setAngle(Rotation2d.fromRadians(desired.theta_rad).minus(trans.getAngle()));
            desired_module_states[i].setLength(desired.drive_output / swerve_kin2.max_output);
            actual_module_states[i].setAngle(Rotation2d.fromRadians(actual.theta_rad).minus(trans.getAngle()));
            actual_module_states[i].setLength(actual.drive_speed_mps / constants.swerve.max_module_speed_mps);
        }
    }
